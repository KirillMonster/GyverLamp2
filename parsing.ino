...
void sync_data(bool sync = false, bool auto_sync = false) {
  if (sync || auto_sync_settings) {
    ...
    packet.clear();
    packet = packet + cfg.bright + "," + cfg.adcMode + "," + cfg.minBright + "," + cfg.maxBright + "," + cfg.rotation + "," + cfg.rotRnd + "," + cfg.rotPeriod + "," + cfg.deviceType + "," + cfg.maxCur + ",";
    packet = packet + cfg.workFrom + "," + cfg.workTo + "," + cfg.matrix + "," + cfg.length + "," + cfg.width + "," + cfg.GMT + "," + cfg.cityID + "," + cfg.mqtt + "," + cfg.mqttID + "," + cfg.mqttHost + ",";
    packet = packet + cfg.mqttPort + "," + cfg.mqttLogin + "," + cfg.mqttPass; 
    DEBUGLN(reply);
    FOR_i(0, 4) {
      sendUDP(reply);
      delay(15);
  ...
  if (Udp.parsePacket()) {
    ...
    switch (data[1]) {
      case 20: DEBUGLN("Sync"); blinkTmr.restart();
        switch (data[2]) {
          case 1:
            auto_sync_settings = data[3];
            sync_data(true, data[3]);
            break;
          case 2:
            auto_sync_settings = data[3];
            DEBUGLN("Set Auto Sync Settings=" + String(data[3]));
            break;
          case 3:
            sendUDP(sync_ver);
